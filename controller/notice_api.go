...
func (d *notice_api) CreateNoticeApi(c *gin.Context) {
	var noticeList []apinotice.SysNotice
...
func (d *notice_api) DeleteNoticeApi(c *gin.Context) {
	...
	//遍历查寻数据是否存在
	var notice apinotice.SysNotice
...
func (d *notice_api) UpdateNoticeApi(c *gin.Context) {
	...
	result := global.Global_Db.Model(&notice).Omit("id").Where("uuid=?", notice.Id).Updates(notice)
	if result.Error != nil {
		// 处理错误
